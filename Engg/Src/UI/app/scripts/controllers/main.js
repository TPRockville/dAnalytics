...
angular.module('jDeriveApp')
  .controller('MainCtrl', function ($scope, basicService, $filter, $interval) {
      $scope.awesomeThings = [
        'HTML5 Boilerplate',
        'AngularJS',
        'Karma'
    ...
    { name: 'Zimbabwe', code: 'ZW' }
      ...
      $scope.drugs = ['A-HYDROCORT', 'A-METHAPRED', 'A-N STANNOUS AGGREGATED ALBUMIN', 'A-POXIDE', 'A.P.L.'];
              ...
              eventsCount.map(function (a) {
                  ...
                  if (a.count > 8000) {
                      ...
                      if ($scope.regions.length > 0 && new Date($scope.regions[$scope.regions.length - 1].end) >= new Date(currentStartDate)) {
                          $scope.regions[$scope.regions.length - 1].end = moment(currentDate).add(moment.duration(5, 'd'));
                      }
                      else {
                          $scope.regions.push({ start: currentStartDate, end: moment(currentDate).add(moment.duration(5, 'd')), class: 'regionYellow' });
                      }
                  }
