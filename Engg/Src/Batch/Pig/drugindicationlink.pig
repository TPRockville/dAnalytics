...
notnulldrugind = FILTER inputfile by ((MEDICINALPRODUCT is not null OR MEDICINALPRODUCT != '') AND (DRUGINDICATION is not null OR DRUGINDICATION != '')) ;

grps = GROUP notnulldrugind by (com.tpgsi.jderive.StringHashCode(UPPER(TRIM(MEDICINALPRODUCT))),com.tpgsi.jderive.StringHashCode(UPPER(TRIM(DRUGINDICATION))));

result = foreach grps generate FLATTEN($0);

STORE result INTO 'CompleteDS/drugindicationlink' USING PigStorage (','); 