...
notnullindications = FILTER inputfile by DRUGINDICATION is not null OR DRUGINDICATION != '';

indications = foreach notnullindications generate com.tpgsi.jderive.StringHashCode(UPPER(TRIM(DRUGINDICATION))),UPPER(TRIM(DRUGINDICATION));

uniqueindication = DISTINCT indications;

STORE uniqueindication INTO 'CompleteDS/drugindication' USING PigStorage (',');